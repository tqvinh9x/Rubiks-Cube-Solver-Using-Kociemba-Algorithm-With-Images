...
namespace TwoPhaseSolver
{
    public static class Search
    {
        ...
        static List<string> axPoToMove(int length, int depthPhase1 = -1)
        {
            ...
            if (depthPhase1 != -1)
            {
                ...
                lst.Add(string.Format(string.Format("{0}\n", p2move)));
                lst.Add(length.ToString());
            }
            return lst;
        }
        ...
        public static List<string> patternSolve(Cube cube, Move pattern, int maxDepth = 30, int timeoutMS = 6000, bool printInfo = false)
        {
            List<string> lst = new List<string>();
            lst = fullSolve(cube, maxDepth, timeoutMS, printInfo);
            return lst;
        }
    }
}
